...
clustering set: $\theta \in \{0.50, 0.60, \ldots, 0.99\}$. The
\texttt{KNN\_COSINE\_THRESHOLD} ($0.60$) and \texttt{TOP\_K} ($40$) were
chosen so that the candidate-edge set has \emph{high} pair recall ($\ge
95\%$ of true positive pairs reachable) at acceptable size ($\sim 600$k
edges for $30$k items).
