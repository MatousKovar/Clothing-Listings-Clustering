...
### Stage 1 — Cosine KNN + connected components ← **BASELINE, implement now**
1. L2-norm joint embeddings (img+txt, 1280d)
2. Batched cosine similarity on MPS, keep top-K=20 neighbours per item
3. Keep edges where `sim > threshold` (sweep 0.80–0.95)
4. Optional: require **mutual KNN** (both directions) to prevent chaining
5. `scipy.sparse.connected_components` → cluster ids
6. **Tune:** threshold on `items_train` pairwise F1

### Stage 2 — Siamese reranking
1. For each edge from Stage 1, score with siamese model → sigmoid
2. Drop edges where siamese score < threshold_2
3. Re-run connected components
4. **Tune:** threshold_2 on `items_train`

### Stage 3 — Cross-cluster merge (if time)
1. For each pair of nearby clusters (centroid cosine > 0.7), check siamese on cross-pairs
2. Merge if enough positive edges

---

## Practical notes
- M4 Pro: use MPS for batched matmul, blocks of 512–1024 rows
- Mutual KNN is the key anti-chaining lever
- Don't block by geo — challenge targets cross-geo duplicates
- Clusters > 100 items: split into chunks of 100

## Validation
Always validate on `items_train.csv` subsample with `evaluate_phase2()` before applying to phase2 data.
